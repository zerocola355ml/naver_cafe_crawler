"""
인기글 전용 뷰어
hot_articles 테이블의 내용을 상세히 확인합니다.
"""
...
def view_hot_articles(show_all=True):
    """
    인기글 목록을 보기 좋게 출력합니다.
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM hot_articles WHERE notification_sent = 1')
        sent = cursor.fetchone()[0]
        
        print(f"\n📊 통계:")
        print(f"   총 인기글: {total}개")
        print(f"   ✅ 알림 발송 완료: {sent}개")
        print(f"   ⏳ 알림 대기 중: {pending}개")
        
        if total == 0:
            print("\n인기글이 없습니다.")
            print("스크래핑을 실행하면 조건을 만족하는 인기글이 자동으로 수집됩니다.")
            return
        ...
        if show_all:
            ...
            query = '''
                SELECT id, article_id, title, comment_count, read_count, like_count, 
                       url, date, detected_at, notification_sent, notification_sent_at
                FROM hot_articles 
                ORDER BY detected_at DESC
            '''
            cursor.execute(query)
        else:
            print(f"\n⏳ 알림 대기 중인 인기글 ({pending}개):")
            query = '''
                SELECT id, article_id, title, comment_count, read_count, like_count, 
                       url, date, detected_at, notification_sent, notification_sent_at
                FROM hot_articles 
        ...
        for idx, article in enumerate(articles, 1):
            (id, article_id, title, comments, reads, likes, 
             url, date, detected, notified, notified_at) = article
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--all":
            view_hot_articles(show_all=True)
        elif sys.argv[1] == "--pending":
            view_hot_articles(show_all=False)
        else:
            print("사용법:")
            print("  python view_hot_articles.py           # 메뉴 모드")
            print("  python view_hot_articles.py --all     # 모든 인기글")
            print("  python view_hot_articles.py --pending # 알림 대기 중만")
    else:
        # 메뉴 모드
        while True:
            choice = show_menu()
